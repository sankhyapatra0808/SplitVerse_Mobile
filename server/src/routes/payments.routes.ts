...
function toRupeesFromPaise(amount: number) {
  return Math.round(amount) / 100;
}
...
async function getWalletBalance(client: Queryable, userId: string) {
...
async function creditWalletForPaidOrder({
  client,
  order,
  payment,
  idempotencyKey,
...
router.post(
  "/razorpay/verify-wallet-payment",
  verifyFirebaseToken,
  async (req: AuthRequest, res) => {
